...
function renderRepos() {
  ...
  function wirePushButton() {
    const button = list.querySelector(".push-committed-button");
    if (!button) return;
    button.addEventListener("click", async () => {
      if (!confirm(`Push committed changes in ${ahead.map((repo) => repo.name).join(", ")} to their tracked branches?`)) return;
      try {
        button.disabled = true;
        const result = await api("/api/push", {repositories: ahead.map((repo) => ({name: repo.name, head: repo.head, upstream: repo.upstream}))});
        notice(`Pushed ${result.pushed.join(", ")}.`, "success");
        invalidatePreview();
        await loadState();
